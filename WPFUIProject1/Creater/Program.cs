...
namespace Creater {
	class Program {
		static void Main(string[] args) {
			...
			string[] ObjType = new string[] { "Control", "Panel", "TextBox" };
			string[] ObjPropety = new string[] { "BG", "FG", "Border" };
			string[] ObjInfoState = new string[] { "Normal", "Disable", }; //"Normal", "Disable", "Toggle", "Imp", "Warning", "Error"
				...
				foreach (var iitem in ObjInfoState) {
					foreach (var uitem in ObjUIState) {
						foreach (var pitem in ObjPropety) {
							...
							Console.WriteLine("<SolidColorBrush x:Key=\"Brush_{0}_{1}_{2}_{3}\" Color=\"#000\"/>", titem, pitem, iitem, uitem);
						}
						Console.WriteLine();
					}
				}
